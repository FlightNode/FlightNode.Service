namespace FlightNode.DataCollection.Domain.Migrations
{
    ...
    public partial class DecimalTemperature : DbMigration
    {
        public override void Up()
        {
            AddColumn("dbo.SurveyCompleted", "Temperature", c => c.Decimal(precision: 18, scale: 2));
            AddColumn("dbo.SurveyPending", "Temperature", c => c.Decimal(precision: 18, scale: 2));

            Sql("UPDATE dbo.SurveyCompleted SET Temperature = StartTemperature");
            Sql("UPDATE dbo.SurveyPending SET Temperature = StartTemperature");

            DropColumn("dbo.SurveyCompleted", "StartTemperature");
            DropColumn("dbo.SurveyCompleted", "EndTemperature");
            DropColumn("dbo.SurveyPending", "StartTemperature");
            DropColumn("dbo.SurveyPending", "EndTemperature");
            
            AlterStoredProcedure("dbo.ExportForagingSurveyResults", @"
BEGIN
    ...
    SELECT 
        ...
        CASE svy.WindDrivenTide WHEN 1 THEN 'true' else 'false' END as WindDrivenTide,
        svy.TimeOfLowTide,
        height.Description as WaterHeight,
        svy.WindSpeed,
        direction.Description as WindDirection,
    ...
    INNER JOIN dbo.WindDirections direction ON svy.WindDirection = direction.Id
    INNER JOIN dbo.WaterHeights height ON svy.WaterHeightId = height.Id
...
END");
        ...
        {
            AddColumn("dbo.SurveyPending", "EndTemperature", c => c.Int());
            AddColumn("dbo.SurveyPending", "StartTemperature", c => c.Int());
            AddColumn("dbo.SurveyCompleted", "EndTemperature", c => c.Int());
            AddColumn("dbo.SurveyCompleted", "StartTemperature", c => c.Int());

            Sql("UPDATE dbo.SurveyCompleted SET StartTemperature = CAST(Temperature as INT)");
            Sql("UPDATE dbo.SurveyPending SET StartTemperature = CAST(Temperature as INT)");

            DropColumn("dbo.SurveyPending", "Temperature");
            DropColumn("dbo.SurveyCompleted", "Temperature");
